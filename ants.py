...
class Place(object):
    ...
    def add_insect(self, insect):
        ...
        """
        if insect.is_ant: # when its the case that insect is an ant
            if self.ant is None: # if there is nothing in the place
                self.ant = insect # ant becomes an insect
            
                # Phase 4: Special handling for BodyguardAnt
                # BEGIN Problem 7
            elif self.ant.can_contain(insect): # if self can contain insect
                self.ant.contain_ant(insect) # then it can
            elif insect.can_contain(self.ant): # otherwise if the insect can contain an ant   
                insect.contain_ant(self.ant) # then it can
                self.ant = insect
            else:
    ...
    def remove_insect(self, insect):
        ...
        if insect.is_ant:
            ...
            if self.ant is insect:
                if hasattr(self.ant, 'container') and self.ant.container and type(self.ant.ant) != QueenAnt:
                    self.ant = self.ant.ant
                elif type(self.ant) == QueenAnt and not self.ant.impostor: #True queen ant can't be removed
                    return
                else:
                    self.ant = None
            else:
                if hasattr(self.ant, 'container') and self.ant.container and self.ant.ant is insect:
                    if type(self.ant.ant)== QueenAnt and not self.ant.ant.impostor: # True queen ant can'tbe removed even in container
                        return
                    self.ant.ant = None
                else:
...
class Ant(Insect):
    ...
    blocks_path = True
    container = False
    doubled = False # becomes True when Queen double the damage
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, hive):
        ...
        counter = 0
        curr_loc = self.place
        while curr_loc.entrance != None or curr_loc != hive:
            if len(curr_loc.bees)>0 and counter <= self.max_range and counter >= self.min_range: 
...
class NinjaAnt(Ant):
    ...
    def action(self, colony):
        # BEGIN Problem 6A
        for element in list(self.place.bees):
...
class QueenAnt(ScubaThrower):  # You should change this line
    ...
    name = 'Queen'
    # BEGIN Problem 9
    # is_imposter becomes True if duplicate is >= 1
    duplicate = 0
    food_cost = 7
    watersafe = True
    impostor = False
    ...
    def __init__(self):
        # BEGIN Problem 9
        Ant.__init__(self)
        if QueenAnt.duplicate >= 1:
            self.impostor = True
            QueenAnt.duplicate = 0
        QueenAnt.duplicate += 1
    ...
    def action(self, colony):
        ...
        #double damage method

        def double_buff(place):
            if type(place.ant) != QueenAnt: # QueenAnt doesn get buffed
                if place.ant is not None and place.ant.doubled== False: # if there is an ant
                    place.ant.damage *=2  # ant damage buffed
                    place.ant.doubled= True # it remains
                if hasattr(place.ant,'container') and place.ant.container and place.ant.ant:
                    if place.ant.ant.doubled == False:
                        place.ant.ant.damage *= 2
                        place.ant.ant.doubled = True

        #implementation of double damage buff

        back = self.place
        while back.exit is not None:
            double_buff(back)
            back= back.exit
...
def make_slow(action):
    ...
    def slow_effect(colony):
        if colony.time % 2 == 0: #calls action method every other turn
            action(colony) # returns action method 
    return slow_effect
...
def make_stun(action):
    ...
    def stun_effect(colony):
        return #actiont that literally does nothing
    return stun_effect
    # END Problem EC
...
def apply_effect(effect, bee, duration):
    ...
    # BEGIN Problem EC
    default_action = bee.action #original action
    affected_action = effect(bee.action) #affected action
